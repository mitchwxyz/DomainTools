...
from src.utils import clean_text, is_valid_url
...
class WebExtractor:
    ...
    def __init__(self, config: Config, max_pages: int | None = None):
        """Initialize the WebExtractor with configuration."""
        self.config = config
        self.visited_urls: set[str] = set()
        self.setup_databases()
        self.setup_http_client()
        self.max_pages = max_pages or self.config.getint("scraper", "max_pages")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    ...
                    return None
                console.print(
                    f"[yellow]Retry {attempt + 1}/{self.max_retries} for {url}: {str(e)}[/yellow]"
                )
        ...
        try:
            ...
            for p in soup.find_all(["p", "article", "section", "div"]):
                ...
                if len(text) > 50 and not any(
                    skip in text.lower() for skip in ["cookie", "accept", "subscribe"]
                ):
    ...
    def find_links(self, html_content: str, base_url: str) -> list[str]:
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if (
                    is_valid_url(next_url, base_domain)
                    and next_url not in self.visited_urls
                    and next_url not in links
                ):
                    links.add(next_url)
    ...
    def process_page(
        self, url: str, extract_jsonld: bool = False, extract_text_content: bool = False
    ) -> tuple[list[str], bool]:
        ...
        console.print(
            f"[{status_color}][{response.status_code}][/{status_color}] {url}"
        )
    ...
    def crawl_site(
        ...
        extract_text_content: bool = False,
    ) -> None:
        ...
        with Progress(
            ...
            *Progress.get_default_columns(),
            ...
            task = progress.add_task("[cyan]Scraping content...", total=self.max_pages)

            while urls_to_visit and len(self.visited_urls) < self.max_pages:
                url = urls_to_visit.pop(0)
                new_urls, success = self.process_page(
                    url, extract_jsonld, extract_text_content
                )
