...
class DataAnalyzer:
    ...
    def _filter_data_by_url(self, url: str, db_type: str = "jsonld") -> list[dict]:
        ...
        return db.search(Query_obj.url.test(lambda x: domain in x))

    def analyze_jsonld(self, url: str | None = None) -> dict:
        """Analyze the JSON-LD data."""
        if url:
            data = self._filter_data_by_url(
                url, db_type="jsonld"
            )  # Specify db_type here
    ...
    def _analyze_jsonld_item(self, data: dict, analysis: dict, depth: int = 0) -> None:
        ...
        if "@type" in data:
            ...
            if isinstance(types, str):
                if depth == 0:
                    ...
                    analysis["nested_types"][types] = (
                        analysis["nested_types"].get(types, 0) + 1
                    )
                    ...
                    if isinstance(type_name, str):
                        if depth == 0:
                            analysis["types"][type_name] = (
                                analysis["types"].get(type_name, 0) + 1
                            )
                        else:
                            analysis["nested_types"][type_name] = (
                                analysis["nested_types"].get(type_name, 0) + 1
                            )
        ...
        if "@context" in data:
            ...
            if isinstance(contexts, str):
                analysis["contexts"][contexts] = (
                    analysis["contexts"].get(contexts, 0) + 1
                )
                ...
                for ctx_key in contexts.keys():
                    if isinstance(ctx_key, str):
                        analysis["contexts"][ctx_key] = (
                            analysis["contexts"].get(ctx_key, 0) + 1
                        )
        ...
        if isinstance(data.get("@type"), str) and data["@type"] in [
            "Organization",
            "Corporation",
            "Company",
        ]:
            ...
            if any(
                t in ["Organization", "Corporation", "Company"] for t in data["@type"]
            ):
                org_name = data.get("name", "Unknown Organization")
                analysis["organizations"][org_name] = (
                    analysis["organizations"].get(org_name, 0) + 1
        ...
        if "datePublished" in data:
            ...
            analysis["dates"]["published"][date] = (
                analysis["dates"]["published"].get(date, 0) + 1
            )
        if "dateModified" in data:
            date = data["dateModified"][:10]  # Get just the date part
            analysis["dates"]["modified"][date] = (
                analysis["dates"]["modified"].get(date, 0) + 1
            )
    ...
    def _extract_author(self, author_data: dict, analysis: dict) -> None:
        ...
        if isinstance(author_data, str):
            analysis["authors"][author_data] = (
                analysis["authors"].get(author_data, 0) + 1
            )
            ...
            analysis["authors"][author_name] = (
                analysis["authors"].get(author_name, 0) + 1
            )
    ...
    def _print_jsonld_analysis(self, analysis: dict, site_info: str) -> None:
        ...
        if analysis["total_pages"] > 0:
            console.print(
                f"Items per Page: {analysis['total_items'] / analysis['total_pages']:.2f}"
            )
        ...
        if analysis["types"]:
            ...
            sorted_types = sorted(
                analysis["types"].items(), key=lambda x: x[1], reverse=True
            )
            for type_name, count in sorted_types[:10]:
                percentage = (count / total_types) * 100
                table.add_row(type_name, str(count), f"{percentage:.1f}%")
            console.print(table)
        ...
        if analysis["authors"]:
            ...
            sorted_authors = sorted(
                analysis["authors"].items(), key=lambda x: x[1], reverse=True
            )
        ...
        if analysis["organizations"]:
            ...
            sorted_orgs = sorted(
                analysis["organizations"].items(), key=lambda x: x[1], reverse=True
            )
        ...
        if analysis["properties"]:
            ...
            sorted_props = sorted(
                analysis["properties"].items(), key=lambda x: x[1], reverse=True
            )
        ...
        if analysis["contexts"]:
            ...
            sorted_contexts = sorted(
                analysis["contexts"].items(), key=lambda x: x[1], reverse=True
            )
    ...
    def analyze_property(self, property_name: str, url: str | None = None) -> dict:
        ...
        def analyze_dict(item: dict, item_url: str, is_nested: bool = False) -> None:
            ...
            if property_name in item:
                ...
                if isinstance(value, (str, int, float, bool)):
                    ...
                    analysis["value_counts"][str(value)] = (
                        analysis["value_counts"].get(str(value), 0) + 1
                    )
                    ...
                    for v in value:
                        if isinstance(v, (str, int, float, bool)):
                            ...
                            analysis["value_counts"][str(v)] = (
                                analysis["value_counts"].get(str(v), 0) + 1
                            )
        ...
        analysis["types_containing_property"] = sorted(
            analysis["types_containing_property"]
        )
        analysis["context_usage"] = sorted(analysis["context_usage"])
        analysis["data_types"] = sorted(analysis["data_types"])
        analysis["urls_containing_property"] = sorted(
            analysis["urls_containing_property"]
        )
    ...
    def _print_property_analysis(self, analysis: dict, site_info: str) -> None:
        ...
        console.print(
            f"Pages Containing Property: {len(analysis['urls_containing_property'])}"
        )
        ...
        if analysis["value_counts"]:
            ...
            sorted_values = sorted(
                analysis["value_counts"].items(), key=lambda x: x[1], reverse=True
            )
            ...
            for rank, (value, count) in enumerate(sorted_values[:10], 1):
                ...
                sample_url = (
                    sample_url[:50] + "..." if len(sample_url) > 50 else sample_url
                )
                    ...
                    def find_types_for_value(d):
                        if isinstance(d, dict):
                            if "@type" in d and analysis["property"] in d:
                                if isinstance(
                                    d[analysis["property"]], (str, int, float, bool)
                                ):
    ...
    def _analyze_page_content(self, page: dict, analysis: dict) -> None:
        ...
        if page.get("meta_description"):
            ...
            analysis["meta_stats"]["avg_description_length"] += len(
                page["meta_description"].split()
            )
        ...
        for para in paragraphs:
            ...
            if length < 50:
                analysis["content_stats"]["paragraph_length_distribution"][
                    "very_short"
                ] += 1
            elif length < 100:
        ...
        if crawl_date:
            ...
            analysis["temporal_stats"]["content_by_date"][crawl_date] += page.get(
                "word_count", 0
            )
    ...
    def _print_text_analysis(self, analysis: dict, site_info: str) -> None:
        ...
        stats_table.add_row(
            ...
            f"{analysis['page_stats']['avg_words_per_page']:.1f}",
        )
        stats_table.add_row(
            "Pages with Headings", str(analysis["page_stats"]["pages_with_headings"])
        )
        stats_table.add_row(
            "Pages with Meta Description",
            str(analysis["meta_stats"]["pages_with_descriptions"]),
        )
        ...
        if analysis["heading_stats"]["common_headings"]:
            ...
            for heading, count in analysis["heading_stats"][
                "common_headings"
            ].most_common(10):
        ...
        if analysis["content_stats"]["paragraph_length_distribution"]:
            ...
            for category, count in sorted(
                analysis["content_stats"]["paragraph_length_distribution"].items()
            ):
        ...
        if analysis["content_stats"]["keyword_density"]:
            ...
            for word, count in analysis["content_stats"]["keyword_density"].most_common(
                20
            ):
        ...
        with Progress(
            ...
            *Progress.get_default_columns(),
            ...
            task = progress.add_task(
                "[cyan]Analyzing subdomains...", total=len(subdomains)
            )
            ...
            for subdomain, ip in subdomains.items():
                try:
                    ...
                    analysis["errors"].append({"subdomain": subdomain, "error": str(e)})
...
def _print_subdomain_content_analysis(self, analysis: dict, base_url: str) -> None:
    ...
    for idx, group in enumerate(analysis["content_groups"], 1):
        ...
        if len(group) > 1:
            ...
            for url1, url2 in combinations(group, 2):
                similarity = fuzz.ratio(
                    url1.get("text", "").lower(), url2.get("text", "").lower()
                )
        ...
        for url_data in group:
            table.add_row(
                url_data["url"],
                str(url_data["title"])[:50] + "..."
                if url_data["title"] and len(url_data["title"]) > 50
                else str(url_data["title"]),
    ...
    if analysis["json_ld_groups"]:
        ...
        for idx, group in enumerate(analysis["json_ld_groups"], 1):
            ...
            for item in group:
                ...
                table.add_row(item["url"], ", ".join(sorted(types)) if types else "N/A")
            console.print(table)
